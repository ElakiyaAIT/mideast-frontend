...
export default function ProductCard({ product }: Props) {
  ...
  return (
    ...
    text-white text-[10px] font-bold px-10 py-1 uppercase shadow`}
      ...
      {/* CONTENT */}
      <div className='p-5'>
        <h3 className='font-bold text-lg mb-4 truncate'>{product.title}</h3>

        <div className='grid grid-cols-3 gap-4 text-sm mb-4'>
          {Object.entries(product.specs).map(([key, value]) => (
            <div key={key}>
              <p className='font-bold'>{value}</p>
              <p className='text-[11px] text-slate-400 capitalize'>{key.replace('_', ' ')}</p>
            </div>
          ))}
        </div>
        ...
        <div className='flex justify-between items-center border-t pt-4'>
          ...
          <button
            ...
            className='bg-orange-400 hover:bg-orange-500 text-white px-5 py-2 rounded-full text-xs font-bold transition'
          >
            {t('product.viewMore')}
          </button>
        </div>
